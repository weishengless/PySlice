"""
Propagation-only benchmark: Build potential/probe once, time only the multislice propagation.
This isolates the core propagation performance from potential building overhead.
"""
import sys,os
sys.path.insert(1,"../../")
...
print("="*50)
print("PROPAGATION-ONLY BENCHMARK")
...
for size in supercell_sizes:
    ...
    # Build potential/probe ONCE (not timed)
    dx = dy = 0.1
    dz = 0.5
    ...
    # Timed propagation (reuse same objects)
    ...
    print(f"    Propagation time: {pyslice_time:.3f}s")
    ...
    # Warmup propagation
    warmup_wave = probe.build()
    warmup_exit = warmup_wave.multislice(potential)
    _ = warmup_exit.compute()
    if device == 'cuda':
        torch.cuda.synchronize()

    # Timed propagation (reuse same potential/probe)
    start = time.perf_counter()
    wave = probe.build()
    ...
    print(f"    Propagation time: {abtem_time:.3f}s")
    del potential, probe, wave, exit_wave, result
    gc.collect()
...
for n_probes in probe_counts:
    ...
    pyslice_time = end - start
    print(f"    Propagation time: {pyslice_time:.3f}s ({pyslice_time/n_probes:.4f}s/probe)")
    ...
    abtem_time = end - start
    print(f"    Propagation time: {abtem_time:.3f}s ({abtem_time/n_probes:.4f}s/probe)")
...
if system_results['sizes']:
    ...
    ax1.set_ylabel('Propagation Time (seconds)', fontsize=12)
...
if probe_results['counts']:
    ...
    ax2.set_ylabel('Propagation Time (seconds)', fontsize=12)
    ax2.set_title('Multi-Probe Scaling (10³ supercell)', fontsize=14, fontweight='bold')
    ax2.legend()
    ax2.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('benchmark_propagation_only.png', dpi=300)
print("\nPlot saved to benchmark_propagation_only.png")
plt.show()

# Print summary
print("\n" + "="*50)
print("PROPAGATION-ONLY BENCHMARK RESULTS")
print("="*50)
